...
class AgentPool extends Subject {
  ...
  async runTest(record) {
    ...
    return result.then(result => {
      ...
      const stderrAllowed = [ 'V8 is running with experimental features enabled. Stability and security will suffer.\n' ];
      if (stderrAllowed.includes(test.rawResult.stderr)) test.rawResult.stderr = '';
